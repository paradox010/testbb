...
export default class SignaturePad {
  ...
  public off(): void {
    ...
    // this.canvas.style.msTouchAction = 'auto'; // 兼容其他浏览器
  ...
  public fromData(pointGroups: IPointGroup[]): void {
    ...
    this._fromData(
      pointGroups,
      ({ color, curve, ifLineEnd }) => this._drawCurve({ color, curve, ifLineEnd }),
  ...
  private _strokeUpdate(event: MouseEvent | Touch): void {
    ...
    if (!lastPoint || !(lastPoint && isLastPointTooClose)) {
      ...
      if (!lastPoint) {
        ...
        this._drawCurve({ color, curve, ifLineEnd: false });
      }
  ...
  private _drawCurve({ color, curve, ifLineEnd }: { color: string; curve: Bezier; ifLineEnd: boolean }): void {
    const ctx = this._ctx;
    ...
    for (const group of pointGroups) {
      ...
      if (points.length > 1) {
        for (let j = 0; j < points.length; j += 1) {
          ...
          if (curve) {
            drawCurve({ color, curve, ifLineEnd: j === points.length - 1 });
          }
        }
      } else {
        this._reset();
  ...
  private _toSVGData(): {
    ...
    this._fromData(
      ...
      ({ color, curve, ifLineEnd }: { color: string; curve: Bezier; ifLineEnd: boolean }) => {
        ...
        if (
          ...
          paths.push({
            strokeWidth: (curve.endWidth * 2.25).toFixed(3),
            stroke: color,
            d: attr,
            strokeLinecap: 'round',
            ifLineEnd,
